...
public:
  ...
  // out に読み込んだ CacheObj を格納する。無効またはエラー時は false を返す。
  bool load(CacheObj& out) const;

  // cache をバイナリ形式で保存する。成功時 true。
  bool save(const CacheObj& cache) const;
